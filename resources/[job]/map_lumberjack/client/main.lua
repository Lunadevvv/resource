...
Citizen.CreateThread(function()
    ...
    --     setTrees(data)
    -- end)
    local blip = AddBlipForCoord(Config.Duty.x, Config.Duty.y, Config.Duty.z)
    SetBlipSprite(blip, 237)
    SetBlipColour(blip, 5)
    SetBlipScale(blip, 0.5)
    SetBlipHighDetail(blip, true)
    BeginTextCommandSetBlipName('STRING')
    AddTextComponentSubstringPlayerName('Tiều phu')
    ...
    if duty then
        for k,v in pairs(trees) do
            if v.health > 0 then
                ...
                SetBlipHighDetail(blip, true)
                BeginTextCommandSetBlipName('STRING')
                AddTextComponentSubstringPlayerName('Cây gỗ')
                EndTextCommandSetBlipName(blip)
                SetBlipAsShortRange(blip, true)
                SetBlipDisplay(blip, 4)
        ...
        if duty then
            ...
            for k,v in pairs(trees) do
                ...
                if (dist < 10) and v.health > 0 then
                    sleep = 6
                    isNear = true
                    Draw3DText(pos.x, pos.y, pos.z + 2, 0.35, '~w~Health: ~o~' .. v.health ..'% ~n~ ~w~[~g~G~w~] ~w~Chặt cây')
                end
            end
...
RegisterCommand('cutatree', function()
    ...
    lib.callback('map_lumberjack:hasItem', false, function(data)
        ...
        AttachEntityToEntity(axe, ped, 
            GetPedBoneIndex(ped, 0x188E), 
            0, 0, 0,
            0, 0, 0,
            true,
            false,
        ...
        lib.callback('map_lumberjack:hasItem', false, function(count)
            if count >= 1 then
                lib.callback('map_lumberjack:makeDamage', false, function(data)
                    if data then
                        TriggerServerEvent('map_lumberjack:reduceAxeDurability')
                    end
                end, closest)
            else
                ESX.ShowNotification('Bạn không có rìu', 'error', 3000)
            end
        end)
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if (dutyPED < 8) then
            sleep = 5
            MessageWithBackground(vector3(Config.Duty.x, Config.Duty.y, Config.Duty.z + 2), '~INPUT_CONTEXT~ <FONT FACE="Baloo Paaji 2 SemiBold">Nhận việc</FONT>')
...
RegisterCommand('startLumberjackDuty', function()
    ...
    lib.callback('map_lumberjack:duty', false, function(boolean)
        duty = boolean
        if (boolean) then
            ESX.ShowNotification('Chúc bạn làm việc thuận lợi', 'success', 3000)
        else
            ESX.ShowNotification('Hẹn gặp lại bạn sau', 'success', 3000)
        end
        lib.callback('map_lumberjack:getTreesWithData', false, function(data)
            setTrees(data)
        end)
