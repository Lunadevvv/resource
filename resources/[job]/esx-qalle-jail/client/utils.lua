RegisterCommand("jailmenu", function(source, args)
	...
	if PlayerData.job.name == "police" then
		...
		ESX.ShowNotification("Ban khong phai la canh sat!")
	end
end)
...
function Cutscene()
	...
	TriggerEvent('esx_anticheat:bypassTLP')
	...
	TriggerEvent('esx_policejob:unhandcuff')
	...
	if #Values["goal"] > 1 then
		...
		function(data, menu)
			...
			if action == "Boiling Broke" or action == "Security" then
				...
				if PlayerData.job.name ~= "police" then
					ESX.ShowNotification("Ban khong co chia khoa!")
					return
				end
			end
