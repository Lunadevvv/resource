...
AddEventHandler('esx_illegal:sellDrug', function(itemName, amount)
	...
	if xItem.count < amount then
		TriggerClientEvent('esx:showNotification', source, _U('dealer_notenough'), 'error', 3000)
		return
	end
	...
	TriggerClientEvent('esx:showNotification', source, _U('dealer_sold', amount, xItem.label, ESX.Math.GroupDigits(price)), 'success', 3000)
end)
