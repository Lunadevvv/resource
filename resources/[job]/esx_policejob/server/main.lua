...
AddEventHandler('esx_policejob:confiscatePlayerItem', function(target, itemType, itemName, amount)
	...
	if itemType == 'item_standard' then
		...
		if targetItem.count > 0 and targetItem.count <= amount then
			...
			if sourceXPlayer.canCarryItem(itemName, sourceItem.count) then
				...
				sourceXPlayer.showNotification(TranslateCap('you_confiscated', amount, sourceItem.label, targetXPlayer.name), 3000, 'success')
				targetXPlayer.showNotification(TranslateCap('got_confiscated', amount, sourceItem.label, sourceXPlayer.name), 3000, 'success')
			else
				sourceXPlayer.showNotification(TranslateCap('quantity_invalid'), 3000, 'error')
			end
		else
			sourceXPlayer.showNotification(TranslateCap('quantity_invalid'), 3000, 'error')
		end
	...
	elseif itemType == 'item_account' then
		...
		if targetAccount.money >= amount then
			...
			sourceXPlayer.showNotification(TranslateCap('you_confiscated_account', amount, itemName, targetXPlayer.name), 3000, 'success')
			targetXPlayer.showNotification(TranslateCap('got_confiscated_account', amount, itemName, sourceXPlayer.name), 3000, 'success')
		else
			sourceXPlayer.showNotification(TranslateCap('quantity_invalid'), 3000, 'error')
		end
	...
	elseif itemType == 'item_weapon' then
		...
		if targetXPlayer.hasWeapon(itemName) then
			...
			sourceXPlayer.showNotification(TranslateCap('you_confiscated_weapon', ESX.GetWeaponLabel(itemName), targetXPlayer.name, amount), 3000, 'success')
			targetXPlayer.showNotification(TranslateCap('got_confiscated_weapon', ESX.GetWeaponLabel(itemName), amount, sourceXPlayer.name), 3000, 'success')
		else
			sourceXPlayer.showNotification(TranslateCap('quantity_invalid'), 3000, 'error')
		end
	end
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_police', function(inventory)
		...
		if count > 0 and inventoryItem.count >= count then
			...
			if xPlayer.canCarryItem(itemName, count) then
				...
				xPlayer.showNotification(TranslateCap('have_withdrawn', count, inventoryItem.name), 3000, 'success')
			else
				xPlayer.showNotification(TranslateCap('quantity_invalid'), 3000, 'error')
			end
		else
			xPlayer.showNotification(TranslateCap('quantity_invalid'), 3000, 'error')
		end
...
AddEventHandler('esx_policejob:putStockItems', function(itemName, count)
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_police', function(inventory)
		...
		if sourceItem.count >= count and count > 0 then
			...
			xPlayer.showNotification(TranslateCap('have_deposited', count, inventoryItem.name), 3000, 'success')
		else
			xPlayer.showNotification(TranslateCap('quantity_invalid'), 3000, 'error')
		end
	end)
...
ESX.RegisterServerCallback('esx_policejob:getOtherPlayerData', function(source, cb, target, notify)
	...
	if notify then
		xPlayer.showNotification(TranslateCap('being_searched'), 3000, 'success')
	end
