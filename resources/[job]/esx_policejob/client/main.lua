...
function setUniform(uniform, playerPed)
	TriggerEvent('skinchanger:getSkin', function(skin)
		...
		if uniformObject then
			...
			ESX.ShowNotification(TranslateCap('no_outfit'), 3000, 'error')
		end
	end)
end
			...
			if Config.EnableESXService then
				ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
					if isInService then
						...
						ESX.ShowNotification(TranslateCap('service_out'), 3000, 'success')
					end
				end, 'police')
			end
		end
			...
			ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
				if not isInService then
					...
					if Config.MaxInService ~= -1 then
						...
						TriggerServerEvent('esx_service:notifyAllInService', notification, 'police')
						TriggerEvent('esx_policejob:updateBlip')
						ESX.ShowNotification(TranslateCap('service_in'), 3000, 'success')
					end
...
function OpenPoliceActionsMenu()
	local elements = {
		...
		{icon = "fas fa-landmark", title = 'Menu Tù', value = 'jail_menu'},
		{icon = "fas fa-exclamation-triangle", title = 'Truy nã', value = 'wanted_menu'}
	...
	ESX.OpenContext("right", elements, function(menu,element)
		...
		if data.current.value == 'citizen_interaction' then
			...
			ESX.OpenContext("right", elements2, function(menu2,element2)
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					ESX.ShowNotification(TranslateCap('no_players_nearby'), 3000, 'error')
				end
			end, function(menu)
				...
				elseif DoesEntityExist(vehicle) then
					...
					elseif action == 'hijack_vehicle' then
						if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 3.0) then
							...
							ESX.ShowNotification(TranslateCap('vehicle_unlocked'), 3000, 'success')
						end
						...
						CreateThread(function()
							while currentTask.busy do
								...
								if not DoesEntityExist(vehicle) and currentTask.busy then
									ESX.ShowNotification(TranslateCap('impound_canceled_moved'), 3000, 'success')
					...
					ESX.ShowNotification(TranslateCap('no_vehicles_nearby'), 3000, 'error')
				end
			end, function(menu)
				OpenPoliceActionsMenu()
			end)
		...
		elseif data.current.value == "wanted_menu" then
			TriggerEvent('wanted:client:menu')
		end
	end)
end
...
function LookupVehicle(elementF)
	...
	ESX.OpenContext("right", elements, function(menu,element)
		...
		if not data.value or length < 2 or length > 8 then
			ESX.ShowNotification(TranslateCap('search_database_error_invalid'), 3000, 'error')
		else
...
function ShowPlayerLicense(player)
	...
	ESX.TriggerServerCallback('esx_policejob:getOtherPlayerData', function(playerData)
		...
		ESX.OpenContext("right", elements, function(menu,element)
			local data = {current = element}
			ESX.ShowNotification(TranslateCap('licence_you_revoked', data.current.label, playerData.name), 3000, 'success')
...
function OpenWeaponComponentShop(components, weaponName, parentShop)
	...
	ESX.OpenContext("right", components, function(menu,element)
		...
		if data.current.hasComponent then
			ESX.ShowNotification(TranslateCap('armory_hascomponent'), 3000, 'success')
		else
			ESX.TriggerServerCallback('esx_policejob:buyWeapon', function(bought)
				if bought then
					if data.current.price > 0 then
						ESX.ShowNotification(TranslateCap('armory_bought', data.current.componentLabel, ESX.Math.GroupDigits(data.current.price)), 3000, 'success')
					end
					...
					ESX.ShowNotification(TranslateCap('armory_money'), 3000, 'success')
				end
	...
	ESX.TriggerServerCallback('esx_policejob:getStockItems', function(items)
		...
		ESX.OpenContext("right", elements, function(menu,element)
			...
			ESX.OpenContext("right", elements2, function(menu2,element2)
				...
				if not count then
					ESX.ShowNotification(TranslateCap('quantity_invalid'), 3000, 'error')
				else
					ESX.CloseContext()
...
ESX.RegisterInput("police:interact", "(ESX PoliceJob) " .. TranslateCap('interaction'), "keyboard", "E", function()
	...
	elseif CurrentAction == 'menu_armory' then
		...
		elseif playerInService then
			...
			ESX.ShowNotification(TranslateCap('service_not'), 3000, 'error')
		end
...
function StartHandcuffTimer()
	...
	handcuffTimer.task = ESX.SetTimeout(Config.HandcuffTimer, function()
		ESX.ShowNotification(TranslateCap('unrestrained_timer'), 3000, 'success')
		TriggerEvent('esx_policejob:unrestrain')
...
function ImpoundVehicle(vehicle)
	...
	ESX.ShowNotification(TranslateCap('impound_successful'), 3000, 'success')
	currentTask.busy = false
end
