            ...
            if not fishing then
                ...
                while fishing do
                    ...
                    if IsDisabledControlJustReleased(0, 24) then
                        TaskPlayAnim(cache.ped, 'mini@tennis', 'forehand_ts_md_far', 1.0, -1.0, 1.0, 48, 0, 0, 0, 0)
                        TriggerEvent('wasabi_fishing:notify', Strings.waiting_bite, Strings.waiting_bite_desc, 'info')
                        Wait(math.random(Config.timeForBite.min, Config.timeForBite.max))
                        TriggerEvent('wasabi_fishing:notify', Strings.got_bite, Strings.got_bite_desc, 'info')
                        Wait(1000)
