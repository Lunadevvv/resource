...
RegisterNetEvent('iens:repair1')
AddEventHandler('iens:repair1', function()
	if isPedDrivingAVehicle() then
		...
		if GetVehicleEngineHealth(vehicle) < cfg.cascadingFailureThreshold + 5 then
			if GetVehicleOilLevel(vehicle) > 0 then
				...
				notification("~g~" .. repairCfg.fixMessages[fixMessagePos] .. ", bây giờ đến một thợ máy!")
				fixMessagePos = fixMessagePos + 1
				if fixMessagePos > repairCfg.fixMessageCount then fixMessagePos = 1 end
			else
				notification("~r~Xe của bạn bị hư hỏng quá nặng. Không thể sửa chữa!")
			end
		else
		...
		notification("~y~Bạn phải ở trong một chiếc xe để có thể sửa chữa nó")
	end
end)
	...
	if isPedDrivingAVehicle() then
		...
		if IsNearMechanic() then
			if GetIsVehicleEngineRunning(vehicle) then
				notification("~g~Động cơ phải được tắt để sửa chữa")
				return
			else
				...
				if PlayerData.job.name == 'mecano' then
					...
					healthPetrolTankLast=1000.0
					SetVehicleEngineOn(vehicle, true, false )
					notification("~g~Bạn đã sửa xe!")
					return			
				else
					if mechNumb == 1 then
						notification("~g~Thợ máy Dave đang xem xét chiếc xe của bạn")
						Citizen.Wait(11000)
						notification("~g~Dave đang làm việc trên ô tô của bạn")
						Citizen.Wait(11000)
						if GetIsVehicleEngineRunning(vehicle) then
							notification("~g~Động cơ phải tắt để sửa chữa")
							return
						else
							SetVehicleUndriveable(vehicle,false)
							SetVehicleFixed(vehicle)
							...
							if cfg.chargeForRepairs then
								...
								notification("~g~Dave đã sửa xe của bạn giá $" .. repairCost .. "!")
								return
							else
								notification("~g~Dave đã sửa chữa ô tô của bạn!")
								return
							end
						end
					elseif mechNumb == 2 then
						notification("~g~Mike thợ máy đang xem xét chiếc xe của bạn")
						Citizen.Wait(11000)
						notification("~g~Mike có vẻ bối rối")
						Citizen.Wait(11000)
						notification("~g~Mike bắt đầu đánh mọi thứ bằng một cái búa")
						Citizen.Wait(11000)
						notification("~g~Mike đi tìm sự giúp đỡ")
						Citizen.Wait(11000)
						notification("~g~Mike's Manager quay lại và bắt đầu làm việc trên ô tô của bạn")
						Citizen.Wait(11000)	
						notification("~g~Người quản lý cũng đang đánh mọi thứ bằng một cái búa")
						Citizen.Wait(11000)	
						if GetIsVehicleEngineRunning(vehicle) then
							notification("~g~Động cơ phải tắt để sửa chữa")
							return
							...
							if cfg.chargeForRepairs then
								TriggerServerEvent('rvFailure:takemoney', repairCost)
								notification("~g~Người quản lý đã sửa chữa ô tô của bạn cho $" .. repairCost .. "!")
								return
							else
								notification("~g~gười quản lý đã sửa chữa ô tô của bạn!")
								return
							end
						end
					elseif mechNumb == 3 then
						notification("~g~Jeff thợ máy đang xem xe của bạn")
						Citizen.Wait(11000)
						notification("~g~JJeff hét lên để Dave đến xem nót")
						Citizen.Wait(11000)
						notification("~g~Chỉ cần nhìn vào nó")
						Citizen.Wait(11000)
						notification("~g~Dave đang làm việc trên ô tô của bạn")
						Citizen.Wait(11000)	
						if GetIsVehicleEngineRunning(vehicle) then	
							...
							if cfg.chargeForRepairs then
								TriggerServerEvent('rvFailure:takemoney', repairCost)
								notification("~g~đã sửa xe của bạn giá $" .. repairCost .. "!")
								return
							else
								notification("~g~Đã sữa xe của bạn!")
								return
							end
						end
					end
...
AddEventHandler('iens:notAllowed', function()
	notification("~r~Bạn không được phép sửa xe")
end)
