...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        local closestVehicle, Distance = ESX.Game.GetClosestVehicle(GetEntityCoords(ped))
        ...
        if cansee1 == false then Wait(1000) end    
        else
    ...
    while true do 
        ...
        if Vehicle.Vehicle ~= nil then
                 if IsVehicleSeatFree(Vehicle.Vehicle, -1) and GetVehicleEngineHealth(Vehicle.Vehicle) <= Config.DamageNeeded then
                    if GetDistanceBetweenCoords(coords, Vehicle.Coords.x, Vehicle.Coords.y, Vehicle.Coords.z, true) < 10.0 then 
                        cansee = true
                        DrawText3D({x = Vehicle.Coords.x, y = Vehicle.Coords.y, z = Vehicle.Coords.z}, 'Ấn [~g~SHIFT~w~] and [~g~E~w~] để đẩy xe', 0.7)
                    end
                end
