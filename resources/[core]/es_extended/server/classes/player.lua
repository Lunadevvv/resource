...
function CreateExtendedPlayer(playerId, identifier, group, accounts, inventory, weight, job, loadout, name, coords, metadata)
	...
	function self.AddCoinLock(amount) 
		MySQL.scalar("SELECT `coinlock` FROM `users` WHERE `identifier` = ?", {self.identifier}, function(result)
			if result then
				print(result)
				MySQL.prepare(
					'UPDATE `users` SET `coinlock` = ? WHERE `identifier` = ?',
					{result + amount, self.identifier},
					function(affectedRows)
					if affectedRows == 1 then
						print(('[^2INFO^7] Add Coin Lock ^5"%d^7"'):format(amount))
					end
					end
				)
			end
	...
	function self.SetCoinLock(amount) 
		MySQL.prepare(
			'UPDATE `users` SET `coinlock` = ? WHERE `identifier` = ?',
			{amount, self.identifier},
			function(affectedRows)
			if affectedRows == 1 then
				print(('[^2INFO^7] Set Coin Lock ^5"%d^7"'):format(amount))
			end
			end
		)
	end

	function self.DelCoinLock(amount) 
		MySQL.scalar("SELECT `coinlock` FROM `users` WHERE `identifier` = ?", {self.identifier}, function(result)
			if result then
				print(result)
				MySQL.prepare(
					...
					function(affectedRows)
					if affectedRows == 1 then
						print(('[^2INFO^7] Delete Coin Lock ^5"%d^7"'):format(amount))
					end
					end
				)
			end
	...
	function self.AddCoin(amount) 
		MySQL.scalar("SELECT `coin` FROM `users` WHERE `identifier` = ?", {self.identifier}, function(result)
			if result then
				MySQL.prepare(
					'UPDATE `users` SET `coin` = ? WHERE `identifier` = ?',
					{result + amount, self.identifier},
					function(affectedRows)
					if affectedRows == 1 then
						print(('[^2INFO^7] Add Coin ^5"%d^7"'):format(amount))
					end
					end
				)
			end
	...
	function self.SetCoin(amount) 
		MySQL.prepare(
			'UPDATE `users` SET `coin` = ? WHERE `identifier` = ?',
			{amount, self.identifier},
			function(affectedRows)
			if affectedRows == 1 then
				print(('[^2INFO^7] Set Coin ^5"%d^7"'):format(amount))
			end
		end
		)
	end

	function self.DelCoin(amount) 
		MySQL.scalar("SELECT `coin` FROM `users` WHERE `identifier` = ?", {self.identifier}, function(result)
			if result then
				print(result)
				MySQL.prepare(
					...
					function(affectedRows)
					if affectedRows == 1 then
						print(('[^2INFO^7] Delete Coin ^5"%d^7"'):format(amount))
					end
				end
				)
			end
