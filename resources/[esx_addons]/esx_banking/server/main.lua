...
AddEventHandler('esx_banking:doingType', function(typeData)
    ...
    if amount == nil or (not typeData.pincode and amount <= 0) then
        TriggerClientEvent("esx:showNotification", source, TranslateCap('invalid_amount'), "error", 3000)
    else
        ...
        elseif typeData.transfer then
            ...
            if tonumber(typeData.transfer.playerId) <= 0 then
                TriggerClientEvent("esx:showNotification", source, TranslateCap("cant_do_it"), "error", 3000)
                return
            end

            if bankMoney < amount then
                TriggerClientEvent("esx:showNotification", source, TranslateCap('not_enough_money', amount), "error", 3000)
                return
            end
        ...
        if typeData.transfer then
            TriggerClientEvent("esx:showNotification", source,
                TranslateCap(string.format('%s_money', key), amount, typeData.transfer.playerId), "success", 3000)
        else
            TriggerClientEvent("esx:showNotification", source, TranslateCap(string.format('%s_money', key), typeData.pincode and (string.format("%04d", amount)) or amount), "success", 3000)
        end
