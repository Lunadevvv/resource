...
function Parkingthecar()
    ...
    ESX.TriggerServerCallback("esx_garage:Parkingcar", function(result)
        if result then
            ...
            ESX.ShowNotification(("Phương tiện [%s] đã được cất vào garage"):format(vehPlate), 'success', 3000)
        else
            ESX.ShowNotification("Phương tiện này không phải của bạn!", 'error', 3000)
        end
    end, vehPlate, vehProps, thisGarage)
end
function SpawnVehicle(data)
    if thisGarage == nil then
        ESX.ShowNotification('Bạn đã đi quá xa!', 'error', 3000)
        return
    end
    ...
    if tonumber(fomat.stored) == 1 then
        ...
        if ESX.Game.IsSpawnPointClear(Config.Garages[thisGarage].SpawnPoint, 3.0) then
            ...
            ESX.ShowNotification("Kẹt xe rồi!", 'error', 3000)
            return
        end
    elseif tonumber(fomat.stored) == 0 then
		...
		ESX.ShowNotification("Bạn đã trả " .. Config.ImpoundPay .."!", 'success', 3000)
