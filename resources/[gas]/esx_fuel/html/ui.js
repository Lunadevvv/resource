$(document).ready(function(){
  ...
  function myStart() {
      if (inv == undefined) { inv = setInterval(increase,400); }
    }
  ...
  $( "#retirar" ).click(function() {
    if (!counting) {
      ...
      completar = !completar;
      maxFuel = 99
      $('.modal span').text("Bạn có muốn đổ $"+(perc_new*dataGas.price).toFixed(2));  
      $('.modal').fadeIn(300);
      $('.container').fadeOut();
      $('left').fadeOut();
      $('.act').fadeOut();
    ...
    if (!counting) {
      ...
      if (price > dataGas.price) {  
        if (price <= ((100-Math.round(datafuel))*dataGas.price)) {
          ...
          $('.modal span').text("Bạn có muốn đổ $ "+price); 
          $('.modal').fadeIn(300);
          $('.container').fadeOut();
          $('left').fadeOut();
          $('.act').fadeOut();
        } else {
          var texto = "Số lượng tối đa <b>$"+((100-Math.round(datafuel))*dataGas.price)+"</b>!"
