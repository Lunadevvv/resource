...
RegisterCommand('ca', function() 
    ...
    if not isDead and not IsPedFalling(ped) and canca then
        ClearPedTasks(GetPlayerPed(-1))
        TriggerEvent("esx_basicneeds:quitsmoking")
        TriggerEvent("dpemotes:clearprops")
        ...
        if not isDoingAction then
            ...
            Citizen.CreateThread(function ()
                while isDoingAction do
                    ...
                    if IsEntityDead(GetPlayerPed(-1)) and mythic_action.canCancel then
                        TriggerEvent("mythic_progbar:client:cancel")
                    end
                end
                if finish ~= nil then
